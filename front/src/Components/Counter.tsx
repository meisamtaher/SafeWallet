...
const Counter: React.FC<Props> = ({ smartAccount, provider }) => {
  ...
  const [name, setName] = useState<string>("Not Initialized");
  const [counterContract, setCounterContract] = useState<any>(null);
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const counterAddress = "0x23A8415a64262ba03fc3ABD5CefaFc80842568A1"//import.meta.env.VITE_COUNTER_CONTRACT_ADDRESS;
  ...
  const getCount = async (isUpdating: boolean) => {
    console.log("Printing get Count!!!!!!");
    const contract = new ethers.Contract(counterAddress, abi, provider);
    setCounterContract(contract);
    console.log("contrac is here )OOOOOO)")
    const currentCount = await contract.count();
    console.log("Count on smart contract: ", currentCount);
